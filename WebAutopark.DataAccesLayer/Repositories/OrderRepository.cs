...
namespace WebAutopark.DataAccesLayer.Repositories
{
    public class OrderRepository : BaseRepository, IRepository<Order>
    {
        ...
        public async Task<IEnumerable<Order>> GetAll() => await Connection.QueryAsync<Order>(QueryGetAll);
        public async Task<Order> GetById(int id) => await Connection.QueryFirstOrDefaultAsync<Order>(QueryGetById, id);
