...
namespace WebAutopark.DataAccesLayer.Repositories
{
    public class DetailRepository : BaseRepository, IRepository<Detail>
    {
        ...
        public async Task Create(Detail item) 
            => await Connection.ExecuteAsync(QueryCreate, item);

        public async Task Delete(int id) 
            => await Connection.ExecuteAsync(QueryDelete, new { DetailId = id });

        public async Task<IEnumerable<Detail>> GetAll() 
            => await Connection.QueryAsync<Detail>(QueryGetAll);

        public async Task<Detail> GetById(int id) 
            => await Connection.QueryFirstOrDefaultAsync<Detail>(QueryGetById, new { DetailId = id});

        public async Task Update(Detail item) 
