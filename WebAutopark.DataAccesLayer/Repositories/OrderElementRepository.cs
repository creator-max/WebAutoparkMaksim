...
namespace WebAutopark.DataAccesLayer.Repositories
{
    public class OrderElementRepository : BaseRepository, IRepository<OrderElement>
    {
        ...
        public async Task<IEnumerable<OrderElement>> GetAll() => await Connection.QueryAsync<OrderElement>(QueryGetAll);
        public async Task<OrderElement> GetById(int id) => await Connection.QueryFirstOrDefaultAsync<OrderElement>(QueryGetById, id);
